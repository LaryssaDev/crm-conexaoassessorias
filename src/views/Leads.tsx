...
export const Leads: React.FC = () => {
  const { user } = useAuth();
  const { leads, addLead, deleteLead } = useData();
  ...
  const handleAddLead = (e: React.FormEvent) => {
    ...
    const lead: Lead = {
      ...
      ...newLead,
      installmentValue: Number(newLead.installmentValue),
      status: 'Novo',
    ...
    setNewLead({ name: '', cpf: '', phone: '', email: '', origin: 'Instagram', contractType: 'Assessoria', installmentValue: '' });
  };
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredLeads.map((lead) => (
                <tr key={lead.id} className="hover:bg-slate-50/50 transition-colors group">
                  ...
                  <td className="px-6 py-4">
                    <div>
                      <p className="text-sm text-slate-800 font-medium">{lead.contractType}</p>
                      <p className="text-xs text-slate-500">R$ {lead.installmentValue.toLocaleString()}</p>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button 
                        onClick={() => alert('Editar lead: ' + lead.name)}
